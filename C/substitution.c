...
int main(int argc, char *argv[])
{
    ...
    if (argc != 2)
    {
        printf("Wrong inputs. ./substitution [key]\n");
        return 1;
    }
    else if (strlen(argv[1]) != 26)
    {
        printf("Key is of wrong length. Must be 26 chars \n");
        return 1;
    }
    else
    {
    ...
    {
        //if lowercase capitalise
        if (96 < key[i] && key[i] < 123)
        {
            key[i] = key[i] - 32;
        }

        //if not within bounds of capital leters, return error
        if (!(64 < key[i] && key[i] < 91))
        {
            printf("Key cannot contain non alphabetic chars \n");
            return 1;
        }

        //check if letter in key[i] has already been used, if it has return error, if not record.
        if (checked[(key[i] - 65)])
        {
            printf("Duplicated chars in key.");
            return 1;
        }
        else
        {
            checked[(key[i] - 65)] = true;
        }
    }
...
char *sub(char key[26])
{
    ...
    for (int i = 0; i < strlen(plaintext); i++)
    {
        ...
        if (c)
        {
            outtext[i] = plaintext[i] - modifier[index];
        }
        else
        {
            outtext[i] = plaintext[i];
        }
    }
